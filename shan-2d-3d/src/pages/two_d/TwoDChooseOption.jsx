  ...
  return (
    <div className="flex justify-around items-center px-3 py-3 mb-3 gap-4">
      ...
      <Modal
        ...
        centered
        contentClassName="!bg-gray-800/95 !backdrop-blur-xl !border !border-gold/30 !shadow-2xl"
        backdropClassName="!bg-black/70 !backdrop-blur-sm"
        style={{ zIndex: 9999 }}
      >
        <Modal.Header closeButton className="!border-b-0 !bg-gray-800/80">
          <Modal.Title>
            <span className="font-semibold text-base text-white">ခွေမည့်ဂဏန်းများ ရိုက်ထည့်ပါ</span>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="flex flex-col gap-4 p-6 !bg-gray-800/90">
          <span className="text-center font-bold text-xs text-gray-300">
            တူညီသော ဂဏန်း(၂)လုံး ခွင့်မပြုပါ။ (ဥပမာ - 020, 131,...)
          </span>
          <input
            className="w-full border-2 border-gold/30 rounded-lg px-3 py-2 text-base bg-gray-700/80 text-white placeholder-gray-400 focus:border-gold/50 focus:outline-none"
            placeholder="ဂဏန်း ၂လုံးမှ ၁၀လုံးအထိ"
          />
        </Modal.Body>
        <Modal.Footer className="flex justify-end gap-3 p-4 !bg-gray-800/80">
          <button
            className="px-4 py-2 rounded-lg bg-gray-600 text-white font-semibold shadow hover:bg-gray-500 transition-all duration-300"
            onClick={() => setIsOp1Show(false)}
          >
            ဖျက်မည်
          </button>
          <button
            className="px-4 py-2 rounded-lg bg-gradient-to-r from-gold to-dark-gold text-gray-900 font-semibold shadow-lg hover:from-gold/90 hover:to-dark-gold/90 transition-all duration-300 hover:scale-105"
            onClick={() => setIsOp1Show(false)}
          >
            အိုကေ
          </button>
      ...
      <Offcanvas
        ...
        className="!bg-gray-800/95 !backdrop-blur-xl"
        backdropClassName="!bg-black/70 !backdrop-blur-sm"
        style={{ zIndex: 9999, bottom: '250px' }}
      >
        <Offcanvas.Header closeButton className="!border-b-0 !bg-gray-800/80">
          <Offcanvas.Title className="font-bold text-lg text-white">အမြန်ရွေး</Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body className="p-4 space-y-6 max-h-[65vh] overflow-y-auto !bg-gray-800/90 pb-6">
          <div>
            <p className="font-bold mb-2 text-lg" style={{ color: '#FFD700' }}>ဘရိတ်</p>
            <div className="flex flex-wrap gap-2 mb-4">
              {breakNums.map((num, index) => {
                return (
                  <button
                    key={index}
                    className="px-4 py-2 rounded-lg bg-gradient-to-r from-gold/30 to-gold/20 font-bold shadow-lg hover:from-gold/40 hover:to-gold/30 border border-gold/60 transition-all duration-300 hover:scale-105 text-base"
                    style={{ color: '#FFD700' }}
          ...
          <div>
            <p className="font-bold mb-2 text-lg" style={{ color: '#FFD700' }}>Single & Double Size</p>
            <div className="flex overflow-x-auto flex-nowrap gap-x-2 pb-2 mb-4">
              {sizes.map((size, index) => (
                <button
                  key={index}
                  className="px-4 py-2 rounded-lg bg-gradient-to-r from-gold/30 to-gold/20 font-bold shadow-lg hover:from-gold/40 hover:to-gold/30 border border-gold/60 transition-all duration-300 hover:scale-105 shrink-0 text-base"
                  style={{ color: '#FFD700' }}
          ...
          <div>
            <p className="font-bold mb-2 text-lg" style={{ color: '#FFD700' }}>20 ဂဏန်း</p>
            <div className="flex flex-wrap gap-2 mb-4">
              {kwat20.map((n) => (
                <button
                  key={n}
          ...
          <div>
            <p className="font-bold mb-2 text-lg" style={{ color: '#FFD700' }}>--</p>
            <div className="flex flex-wrap gap-2 mb-4">
              {kwat20.map((n) => (
                <button
                  key={n}
