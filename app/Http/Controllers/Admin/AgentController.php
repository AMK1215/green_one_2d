...
class AgentController extends Controller
{
     ...
     */
    public function getCashIn(string $id): Response
    {
        ...
        return Inertia::render('admin/Agents/CashIn', [
            'agent' => $agent,
            'ownerBalance' => $owner->balanceFloat,
        ]);
    }

    public function getCashOut(string $id): Response
    {
        // if (! Gate::allows('make_transfer')) {
        //     abort(403);
        // }
        ...
        return Inertia::render('admin/Agents/CashOut', [
            'agent' => $agent,
    ...
    public function makeCashIn(Request $request, $id): RedirectResponse
    {
        ...
        try {
            ...
            return redirect()->route('admin.agents.index')->with('success', 'Money transferred successfully!');
        } catch (Exception $e) {
    ...
    public function makeCashOut(TransferLogRequest $request, string $id): RedirectResponse
    {
        ...
        try {
            ...
            return redirect()->route('admin.agents.index')->with('success', 'Money withdrawn successfully!');
        } catch (Exception $e) {
    ...
    public function getTransferDetail($id): Response
    {
        // abort_if(
        ...
        return Inertia::render('admin/Agents/TransferLogs', [
            'agent' => $agent,
    ...
    {
        $users = User::with('roles', 'twoBets', 'results', 'betNResults')
    ...
    {
        // Eager-load agent, roles, and children (downlines) with their twoBets relationship
        $agent = User::with([
            'roles',
            'children.twoBets',
        ])->findOrFail($id);

        // Sum all win_lose_amt for all twoBets of all children
        $poneWineTotalAmt = $agent->children
            ->flatMap(function ($child) {
                return $child->twoBets;
            })
