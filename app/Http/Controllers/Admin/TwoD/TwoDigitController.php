...
class TwoDigitController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        return Inertia::render('admin/2D/Reports', [
            'bets' => $bets,
            'session' => $session,
            'date' => $date,
        ]);
    ...
    public function betSlipList(Request $request): Response
    {
        ...
        $slips = $query->with('user')->latest()->paginate(30);

        return Inertia::render('admin/2D/BetSlips', [
            'slips' => $slips,
            'session' => $session,
            'date' => $date,
        ]);
    ...
    public function betSlipDetails($slip_id): Response
    {
        $user = auth()->user();
        ...
        return Inertia::render('admin/2D/BetSlipDetails', [
            'bets' => $bets,
            'slip' => $slip,
        ]);
    }
    ...
    // head close digit
    public function headCloseDigit(): Response
    {
        ...
        return Inertia::render('admin/2D/Settings', [
            'headCloseDigits' => $headCloseDigits,
            'chooseCloseDigits' => $chooseCloseDigits,
            'battles' => $battles,
            'twoDLimit' => $twoDLimit,
    ...
    public function toggleStatus(Request $request)
    {
        try {
            ...
            Log::info('Toggle HeadClose Request:', [
                'id' => $request->id,
            ...
            if (!$digit) {
                Log::error('HeadClose not found:', [
                    'requested_id' => $request->id,
                    'available_ids' => HeadClose::pluck('id')->toArray()
                ]);
                return back()->withErrors(['error' => 'Digit not found.']);
            }

            $digit->status = $request->status;
            $digit->save();

            Log::info('HeadClose status updated successfully:', [
                'id' => $digit->id,
                'digit' => $digit->head_close_digit,
                'new_status' => $digit->status
            ]);

            return back()->with('success', 'Head close digit status updated successfully.');

        } catch (\Exception $e) {
            Log::error('Failed to toggle HeadClose status: '.$e->getMessage(), [
                'digit_id' => $request->id,
                'requested_status' => $request->status,
                'exception' => $e->getTraceAsString(),
            ]);

            return back()->withErrors(['error' => 'An internal server error occurred.']);
        }
    }
    ...
    public function toggleChooseDigitStatus(Request $request)
    {
        try {
            // Log the incoming request for debugging
            Log::info('Toggle ChooseDigit Request:', [
                'id' => $request->id,
                'status' => $request->status,
                'all_data' => $request->all()
            ]);
            ...
            if (!$digit) {
                Log::error('ChooseDigit not found:', [
                    'requested_id' => $request->id,
                    'available_ids' => ChooseDigit::pluck('id')->toArray()
                ]);
                return back()->withErrors(['error' => 'Digit not found.']);
            }
            ...
            Log::info('ChooseDigit status updated successfully:', [
                'id' => $digit->id,
                'digit' => $digit->choose_close_digit,
                'new_status' => $digit->status
            ]);

            return back()->with('success', 'Choose digit status updated successfully.');
     ...
     */
    public function toggleBattleStatus(Request $request)
    {
        try {
            $request->validate([
                'id' => 'required|integer',
            ...
            if (!$battle) {
                return back()->withErrors(['error' => 'Battle time not found.']);
            }

            $battle->status = $request->status;
            $battle->save();

            return back()->with('success', 'Battle status updated successfully.');
    ...
    public function storeTwoDLimit(Request $request)
    {
        ...
        return redirect()->route('admin.2d.settings')->with('success', 'TwoD Limit (Break) added successfully.');
    }
    ...
    public function storeTwoDResult(Request $request)
    {
        ...
        return redirect()->route('admin.2d.settings')->with('success', 'TwoD Result added and winners paid.');
    }
    ...
    public function dailyLedger(Request $request): Response
    {
        $user = Auth::user();
        ...
        if ($session === 'morning' || $session === 'evening') {
            ...
            return Inertia::render('admin/2D/Ledger', [
                'result' => $result,
                'date' => $date,
                'session' => $session,
            ]);
    ...
    public function dailyWinners(Request $request): Response
    {
        ...
        if (! $session) {
            ...
            return Inertia::render('admin/2D/Winners', [
                'date' => $date,
