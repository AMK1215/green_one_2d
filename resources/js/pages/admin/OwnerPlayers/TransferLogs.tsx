...
import { ArrowLeft, History, ArrowUpRight, ArrowDownLeft } from 'lucide-react';
...
export default function OwnerPlayerTransferLogs({ player, transferLogs }: Props) {
    ...
    const getTransactionIcon = (transfer: TransferLog) => {
        // If player is the sender (money going out)
        if (transfer.from_user_id === player.id) {
            return <ArrowUpRight className="h-4 w-4 text-red-500" />;
        }
        // If player is the receiver (money coming in)
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Transfer Logs - ${player.name}`} />
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="mt-2 text-gray-600 dark:text-gray-400">
                            Transaction history for {player.name} ({player.user_name})
                        </p>
                    </div>
                </div>
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-red-600">
                                {formatCurrency(
                                    transferLogs.data
                                        ?.filter(log => log.from_user_id === player.id)
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <div className="text-2xl font-bold text-green-600">
                                {formatCurrency(
                                    transferLogs.data
                                        ?.filter(log => log.to_user_id === player.id)
                                        ?.reduce((total, log) => total + log.amount, 0) || 0
                                )}
                            </div>
                        </CardContent>
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Complete transfer history for this player
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                                        ...
                                        <tr key={transfer.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                                            <td className="border border-gray-200 dark:border-gray-700 px-4 py-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <Badge variant={transfer.from_user_id === player.id ? "destructive" : "default"}>
                        ...
                        {(!transferLogs.data || transferLogs.data.length === 0) && (
                            <div className="text-center py-8 text-gray-500">
                                No transfer logs found for this player.
                            </div>
                        )}
                    </CardContent>
                </Card>
